"""Read-only Windows probes: ping, DNS, HTTPS, TCP/443, proxy flags, adapters (stdlib + subprocess).

Does not mutate network state or run repair scripts."""
...
from .features import Features, from_dict as features_from_dict
...
def collect(repo_root: Path | None = None) -> tuple[Features, dict[str, Any]]:
    """Run probes on the local machine and return ``Features`` plus command metadata."""

    del repo_root
    executed: list[dict[str, str]] = []

    def logged(cmd: list[str], label: str) -> tuple[int, str]:
        executed.append({"label": label, "cmd": subprocess.list2cmdline(cmd)})
        return run_command(cmd)

    ping_ip_ok = logged(["ping", "-n", "1", "-w", "2000", "8.8.8.8"], "ping_ip")[0] == 0
    ping_domain_ok = logged(["ping", "-n", "1", "-w", "3000", "google.com"], "ping_domain")[0] == 0
    nslookup_ok = logged(["nslookup", "google.com"], "nslookup")[0] == 0

    executed.append({"label": "tcp_443", "cmd": "Test-NetConnection (PowerShell)"})
    tcp_443_ok = _test_tcp_443()

    executed.append({"label": "https_curl", "cmd": "curl -I https://www.google.com"})
    ...
    user_px = _user_proxy_enabled()
    proxy_enabled = winhttp_on or user_px[0]
    ...
    if gateway_ip:
        gw_code, _ = logged(["ping", "-n", "1", "-w", "2000", gateway_ip], "ping_gateway")
        gateway_reachable = gw_code == 0

    tw = _count_netstat_keyword("TIME_WAIT")
    est = _count_netstat_keyword("ESTABLISHED")
...
def load_fixture(path: Path) -> Features:
    data = json.loads(path.read_text(encoding="utf-8"))
    if not isinstance(data, dict):
        raise ValueError("fixture root must be an object")
    root = data.get("features")
    if isinstance(root, dict):
        return features_from_dict(root)
...
def _https_probe() -> tuple[bool, bool]:
    code, out = run_command(["curl", "-I", "--max-time", "12", "https://www.google.com"], timeout=22.0)
    joined = out.lower()
    tls_hint = any(
        x in joined
        for x in (
...
def _count_netstat_keyword(keyword: str) -> int:
    ...
    for line in out.splitlines():
        ...
        if stripped.startswith("dns servers"):
            ...
            if remainder and remainder.replace(".", "").isdigit():
                count += 1
        elif stripped.startswith("fec0:0:0:ffff::") or re.search(r"^\s+\d{1,3}(\.\d{1,3}){3}\s*$", line):
            count += 1
    return max(count, 0)
