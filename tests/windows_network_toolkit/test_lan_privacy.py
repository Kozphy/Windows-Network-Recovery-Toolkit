...
def test_lan_watch_fixture_replay():
    data = json.loads(
        (LAN_EXAMPLES / "normal_home_network.jsonl").read_text(encoding="utf-8").splitlines()[0]
    )
    payload = run_lan_watch(
        duration=1,
        inject_sequence=[
            {"devices": data.get("devices", []), "observations": data.get("observations", [])}
        ],
    )
